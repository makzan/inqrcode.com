...
| `netlify.toml` | Points the functions directory at `netlify/functions` and pins two Node versions: `NODE_VERSION = "22"` for the build (without it Netlify builds with an ancient default Node and native deps like `canvas` fail to install) and `AWS_LAMBDA_JS_RUNTIME = "nodejs22.x"` for the functions runtime (this site predates Netlify's Node 22 default; old runtimes can't `require()` ES modules and crash with `ERR_REQUIRE_ESM`). |
